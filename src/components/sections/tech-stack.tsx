...
const TechStackSection = () => {
  return (
    <section id="techstack" className="relative bg-black py-32 overflow-hidden border-t border-[#1a1a1a]">
      ...
      <div className="container mx-auto px-8 max-w-[1280px] relative z-10">
        ...
        <div className="mb-16">
          <div className="mb-6 flex items-center gap-2">
            <div className="grid grid-cols-3 gap-1">
              {[...Array(9)].map((_, i) => (
                <div key={i} className={`h-1.5 w-1.5 rounded-full ${i % 2 === 0 ? 'bg-[#0ea5e9]' : 'bg-[#0ea5e9]/30'}`} />
              ))}
            </div>
        ...
        <div className="flex flex-wrap gap-3 mb-24">
          {technologies.map((tech, index) => (
            <div
              key={index}
              className="group relative px-5 py-3 bg-[#0d0d0d] border border-[#1a1a1a] rounded-full transition-all duration-300 hover:border-[#0ea5e9]/50 hover:bg-[#0ea5e9]/5 cursor-default"
            >
              <span className="text-[14px] font-medium text-white group-hover:text-[#0ea5e9] transition-colors">
                {tech.name}
              </span>
              {/* Category tooltip on hover */}
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-[#1a1a1a] rounded text-[10px] text-[#999999] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                {tech.category}
              </div>
            </div>
          ))}
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {passions.map((passion, index) => (
              <div
                key={index}
                className="group p-6 bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl transition-all duration-300 hover:border-[#0ea5e9]/30 hover:bg-[#0ea5e9]/5"
              >
      ...
      {/* Decorative Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#0ea5e9]/5 blur-[150px] rounded-full pointer-events-none" />
    </section>
  );
};
